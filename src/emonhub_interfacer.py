...
class EmonHubInterfacer(object):
    ...
    def _decode_frame(self, ref, data):
        ...
        self._log.debug(" data to decode " + str(data))
        ...
        if node in ehc.nodelist and 'datacodes' in ehc.nodelist[node]:
            ...
            if len(data) != sum(datasizes):
                self._log.warning(str(ref) + "  RX data length: " + str(len(data)) +
                                  " is not valid for datacodes " + str(datacodes) + str(sum(datasizes)))
                return False
            else:
            ...
            # Determine the number of values in the frame of the specified code & size
                ...
                self._log.warning("number of values : "+str(count) )
        ...
        if not decoded:
            ...
            for i in range(0, count, 1):
                ...
                if not datacode:
                    dc = datacodes[i]
                    self._log.debug("datacode "+str(dc))
                # Determine the number of bytes to use for each value by it's datacode
                size = int(ehc.check_datacode(dc))
                self._log.debug(" number of bytes to decode "+str(size) +str( data[bytepos:bytepos+size]))
                try:
                    value = ehc.decode(dc, [int(v) for v in data[bytepos:bytepos+size]])
                    self._log.debug(" data to decode "+str(data[bytepos:bytepos+size]))
                except struct.error:
                    self._log.warning(str(ref) + " Unable to decode as values incorrect for datacode(s)"+struct.error)
                    return False
                bytepos += size
...
class EmonHubJeeInterfacer(EmonHubSerialInterfacer):
    ...
    def _validate_frame(self, ref, received):
        ...
        This lsfunction performs logical tests to filter unsuitable data.
        Each test discards frame with a log entry if False
        ...
        #    self._log.info(  "striped ok from received")
        ...
        #    return received
            ...
            #    self.rssi = False
            #self._log.info(  "oke not found")

        # Strip 'OK' and extract RSSI if packet is from RFM69 type Jee Device
        if received[0]=='OK' :
            self._log.info(  "striped ok from received"+str(received))
            received = received[2:]
            self._log.info(  "after stripping"+str(received))
            return received
        else:
            # set RSSI false for standard frames so RSSI is not re-appended later
            self.rssi = False
